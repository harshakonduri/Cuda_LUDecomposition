...
/*
__global__ void reduce( float *a, int b, int c) {
	extern __shared__ float rowCopy[];
	int tid = blockIdx.x;	//Handle the data at the index
	int thid = threadIdx.x;
	int numthreads = blockDim.x;
	int numblocks = (int) (b/numthreads);
	int index=c,l=0,size=b,j=0,rowpos=0;
	int loop_iter = (int)(size-index)/numblocks;
	for(l=0;l<size;l++) {
	 rowCopy[(index*size)+l] = a[(index*size)+l];
	}
	__syncthreads();

	for(j=index+1;j<size;j+=numthreads) {
		a[((tid+index+1)*size + j+thid)] = (float)(a[((tid+index+1)*size + j+thid)] - (float)a[((tid+index+1)*size+index)] * rowCopy[j+thid]);
	}
...
input = (float *) malloc(sizeof(float*) * n * n);
	...
	//numblocks = (int) ceil(n/numthreads);
	gettimeofday(&bef,NULL);
	for(i = 0;i<n;i++) {								
	scale<<<1, 1>>> (dev_a, n, i);
	reduce<<<n-i-1,numthreads,sizeof(float)*n>>>(dev_a, n, i);
	}
